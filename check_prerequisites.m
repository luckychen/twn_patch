function check_prequisite(annot_fn, quic_dir)
    ...
    try
        [annot annot_ids] = dataset('File', annot_fn, 'ReadObsNames', false, 'ReadVarNames', true, 'Delimiter', '\t');
        iso2gene = containers.Map(cellstr(annot(:, find(strcmp(annot_ids, 'transcript_id'))) ), cellstr(annot(:, find(strcmp(annot_ids, 'gene_id')) )) );
    catch ME
        display(ME.message);
        throw(ME);
    end
