...
[ds1 samples1] = dataset('File', te_fn, 'ReadObsNames', true, 'ReadVarNames', false, 'HeaderLines', 1, 'Delimiter', '\t'); 
...
fh = fopen(te_fn);
first_line = fgets(fh);
genes = regexp(first_line, '[\n\t]', 'split');
genes = genes(2:(size(genes,2)-1));
fclose(fh);


[ds2 samples2] = dataset('File', iso_fn, 'ReadObsNames', true, 'ReadVarNames', false, 'HeaderLines', 1, 'Delimiter', '\t');
...
[annot annot_idx] = dataset('File', annot_fn, 'ReadObsName', false, 'ReadVarNames', true, 'Delimiter', '\t');
iso2gene = containers.Map(cellstr(annot(:, find(strcmp(annot_ids, 'transcript_id'))) ), cellstr(annot(:, find(strcmp(annot_ids, 'transcript_id')) )) );
